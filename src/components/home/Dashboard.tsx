...
export default function Dashboard() {
		...
		const fetchData = async () => {
			try {
				...
				if (!(response instanceof Error)) {
					const spending = response.reduce(
	...
	useEffect(() => {
		listenToUserPreference().then((response) => setCurrency(response));
		// .catch(() => {
		//   if (user) savePreference(user.uid, currency);
		// });
	}, []);
	...
	const aggregateData = (data: Expenses): AggregatedData[] => {
		...
		data.forEach((item) => {
			...
			const sum = -item.sum;
			if (aggregatedData[type]) aggregatedData[type].sum += sum;
			else {
				aggregatedData[type] = {
					type: type,
					sum: sum,
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Your expenses by category for {selectedPeriod}{" "}
								months
							</CardDescription>
						</CardHeader>
						<CardContent>
					...
					<Card>
						<CardHeader>
							...
							{selectedPeriod == 1 ? (
								<CardDescription>
									Your balance for this month
								</CardDescription>
							) : (
								<CardDescription>
									Your balance for {selectedPeriod} months
								</CardDescription>
							)}
						</CardHeader>
						<CardContent>
